...
def read_config(path: str = "config/urls.csv") -> List[InputRow]:
    ...
    with open(path, newline='', encoding='utf-8') as f:
        ...
        for r in reader:
            out.append(InputRow(url=r["url"].strip(), brand=(r.get("brand") or "").strip(), sku=(r.get("sku") or "").strip()))
    return out
...
def resolve_inventory(api: MercadoLibreAPI, row: InputRow) -> Tuple[str, str, str]:
    ...
    if item_id:
        try:
            ...
            return row.sku or item.get("seller_custom_field") or item_id, title_for_report, ("si" if available > 0 else "no")
        except Exception:
            pass
    ...
    if row.sku:
        try:
            ...
            if results:
                ...
                available = int(item.get("available_quantity", 0))
                title_for_report = item.get("title") or row.url
                return row.sku, title_for_report, ("si" if available > 0 else "no")
        except Exception:
            pass

    title, has_stock, _badge = probe_stock_from_page(row.url)
    title_for_report = title or row.url
    value = "si" if has_stock else ("no" if has_stock is False else "")
...
def main():
    ...
    rows: List[Row] = []
    for r in cfg:
        sku, product, value = resolve_inventory(api, r)
